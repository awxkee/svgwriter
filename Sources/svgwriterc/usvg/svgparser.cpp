    ...
    while(!str.isEmpty()) {
        ...
        if(cmd == "matrix" && args.size() == 6)
            ...
            matrix = matrix * Transform2D::rotating(args[0]*M_PI/180,
                                                    args.size() == 3 ? SVGPoint(args[1], args[2]) : SVGPoint(0,0));
        ...
        while(num != end) {
            ...
            switch(tolower(pathElem)) {
                ...
                case 'c': {
                    ...
                    SVGPoint c1(num[0] + offsetX, num[1] + offsetY);
                    SVGPoint c2(num[2] + offsetX, num[3] + offsetY);
                    SVGPoint e(num[4] + offsetX, num[5] + offsetY);
                    num += 6;
                ...
                case 's': {
                    ...
                    if(lastMode == 'c' || lastMode == 'C' || lastMode == 's' || lastMode == 'S')
                        c1 = SVGPoint(2*x - ctrlPt.x, 2*y - ctrlPt.y);
                    else
                        c1 = SVGPoint(x, y);
                    SVGPoint c2(num[0] + offsetX, num[1] + offsetY);
                    SVGPoint e(num[2] + offsetX, num[3] + offsetY);
                    num += 4;
                    path.cubicTo(c1, c2, e);
                    ctrlPt = c2;
                    x = e.x;
                ...
                case 'q': {
                    if(end - num < 4) { num = end; break; }
                    SVGPoint c(num[0] + offsetX, num[1] + offsetY);
                    SVGPoint e(num[2] + offsetX, num[3] + offsetY);
                    num += 4;
                    path.quadTo(c, e);
                    ctrlPt = c;
                ...
                case 't': {
                    if(end - num < 2) { num = end; break; }
                    SVGPoint e(num[0] + offsetX, num[1] + offsetY);
                    num += 2;
                    SVGPoint c;
                    if(lastMode == 'q' || lastMode == 'Q' || lastMode == 't' || lastMode == 'T')
                        c = SVGPoint(2*x - ctrlPt.x, 2*y - ctrlPt.y);
                    else
                        c = SVGPoint(x, y);
                    path.quadTo(c, e);
                    ctrlPt = c;
...
SvgNode* SvgParser::createImageNode()
{
    ...
    return new SvgImage(std::move(image), SVGRect::ltwh(x, y, w, h), target);
}
...
SvgNode* SvgParser::createLineNode()
{
    ...
    return new SvgPath(Path2D().addLine(SVGPoint(x1, y1), SVGPoint(x2, y2)), SvgNode::LINE);
}
...
SvgNode* SvgParser::createRectNode()
{
    ...
    return new SvgRect(SVGRect::ltwh(x, y, w, h), rx, ry);
}
...
SvgDocument* SvgParser::createSvgDocumentNode()
{
    ...
    if(viewBox.size() == 4)
        doc->setViewBox(SVGRect::ltwh(viewBox[0], viewBox[1], viewBox[2], viewBox[3]));
...
SvgNode* SvgParser::createUseNode()
{
    ...
    return new SvgUse(SVGRect::ltwh(x, y, w, h), href.toString().c_str(), link, doc);
}

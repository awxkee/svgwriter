...
#include "svgpainter.hxx"  // only needed for bounds()
...
SVGRect SvgNodeExtension::dirtyRect() const
{
    return node->m_dirty != SvgNode::NOT_DIRTY ? node->bounds().rectUnion(node->m_renderedBounds) : SVGRect();
}
...
SvgNode* SvgContainerNode::nodeAt(const SVGPoint& p, bool visual_only) const
{
...
: m_cell(SVGRect::ltwh(x, y, w, h)), m_patternUnits(pu), m_patternContentUnits(pcu) {}
...
Transform2D SvgDocument::viewBoxTransform() const
{
    ...
    SVGRect source = m_viewBox;
    if(source.isValid()) {
        SVGRect target = viewportRect();
        if(source != target) {
...
SvgImage::SvgImage(Image image, const SVGRect& bounds, const char* linkStr)
...
SVGRect SvgImage::viewport() const
{
    ...
    if(w > 0 && h > 0)
        return m_bounds;
    if(w <= 0 && h <= 0)
        return SVGRect::ltwh(m_bounds.left, m_bounds.top, imgw, imgh);
    if(w > 0)  // h <= 0
        return SVGRect::ltwh(m_bounds.left, m_bounds.top, w, imgh*w/imgw);
    //if(h > 0)  // w <= 0
    return SVGRect::ltwh(m_bounds.left, m_bounds.top, imgw*h/imgh, h);
}
...
SvgRect::SvgRect(const SVGRect& rect, real rx, real ry) : SvgPath(RECT), m_rect(rect), m_rx(rx), m_ry(ry)
{
    updatePath();
}

// if rx (or ry) is not passed or < 0, rx (or ry) is not changed
void SvgRect::setRect(const SVGRect& rect, real rx, real ry)
{
...
SvgUse::SvgUse(const SVGRect& bounds, const char* linkStr, const SvgNode* node, SvgDocument* doc)

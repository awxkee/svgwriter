#include "svgpainter.hxx"
#include "svgstyleparser.hxx"


// perhaps this should be a static method passed a Painter* ?

void SvgPainter::drawNode(const SvgNode* node, const SVGRect& dirty)
{
...
SVGRect SvgPainter::nodeBounds(const SvgNode* node)
{
...
std::vector<SVGRect> SvgPainter::glyphPositions(const SvgText* node)
{
    p->save();
    initPainter();
    ...
    real lineh = 0;
    SVGRect r;
    std::vector<SVGRect> positions;
    drawTextTspans(node, SVGPoint(0,0), &lineh, &r, &positions);
    extraStates.clear();
    p->restore();
    return positions;
}
...
SVGRect SvgPainter::calcDirtyRect(const SvgNode* node)
{
    SVGRect dirty;
...
void SvgPainter::clearDirty(const SvgNode* node)
{
    if(node->m_dirty != SvgNode::NOT_DIRTY) {
        ...
        const SvgContainerNode* container = node->asContainerNode();
        if(container) {
            container->m_removedBounds = SVGRect();
            for(SvgNode* child : container->children())
                clearDirty(child);
        }
    }
    ...
    for(const SvgNode* child : node->children()) {
        ...
        if(!child->isVisible()) {
            ...
            if(child->type() == SvgNode::DEFS || child->type() == SvgNode::SYMBOL)
                ...
                child->m_renderedBounds = SVGRect();
        }
...
SVGRect SvgPainter::bounds(const SvgNode* node)
{
...
SVGRect SvgPainter::childrenBounds(const SvgContainerNode* node)
{
    SVGRect b;
...
Brush SvgPainter::gradientBrush(const SvgGradient* gradnode, const SvgNode* dest)
{
    ...
    if(grad.coordinateMode() == Gradient::ObjectBoundingMode && dest->cachedBounds().isValid()) {
        SVGRect localBBox = (p->getTransform().inverse() * initialTransform).mapRect(dest->cachedBounds());
        grad.setObjectBBox(localBBox);
    }
    else
        grad.setObjectBBox(SVGRect());
    Brush b(&grad);
...
{
    SVGRect dest = path->boundingRect();
    real destw = dest.width();
    real desth = dest.height();
    SVGRect cell = pattern->m_cell;

    p->save();
    Transform2D tfin = p->getTransform();
    SVGRect clipin = p->getClipBounds();
    ...
    if(pattern->m_patternUnits == SvgPattern::objectBoundingBox)
        cell = SVGRect::ltwh(cell.left*destw, cell.top*desth, cell.width()*destw, cell.height()*desth);
    ...
    SVGRect localDirty = (p->getTransform().inverse() * initialTransform).mapRect(dirtyRect);
    real w = cell.width();
    real h = cell.height();
...
void SvgPainter::_draw(const SvgUse* node)
{
    ...
    if(!target)  // should not normally happen since node will have invalid bounds
        return;
    SVGRect m_bounds = node->viewport(); //m_bounds;
    ...
    SVGRect nodebounds = node->cachedBounds();
...
void SvgPainter::_draw(const SvgText* node)
{
    ...
    drawTextTspans(node, SVGPoint(0,0), &lineh);
}
...
SVGRect SvgPainter::_bounds(const SvgDocument* doc)
{
...
SVGRect SvgPainter::_bounds(const SvgImage* node)
{
    return p->getTransform().mapRect(node->viewport());
}

SVGRect SvgPainter::_bounds(const SvgPath* node)
{
    ...
    if(node->pathType() == SvgNode::RECT)
        return p->getTransform().mapRect(SVGRect(static_cast<const SvgRect*>(node)->m_rect).pad(strokewidth/2));
    if(node->m_path.empty())
        return SVGRect();
    // I think we can just map the bounding rect if there is no rotation ... probably should add some tests!
    SVGRect b = !tf.isRotating() ? tf.mapRect(node->m_path.boundingRect())
...
SVGRect SvgPainter::_bounds(const SvgUse* node)
{
    const SvgNode* target = node->target();
    if(!target)
        return SVGRect();
...
SVGRect SvgPainter::_bounds(const SvgCustomNode* node)
{
    return node->hasExt() ? node->ext()->bounds(this) : SVGRect();
}

SVGRect SvgPainter::_bounds(const SvgText* node)
{
    real lineh = 0;
    SVGRect r;
    drawTextTspans(node, SVGPoint(0,0), &lineh, &r);
    return r;
}
...
SVGPoint SvgPainter::drawTextText(const SvgTspan* node, SVGPoint pos, real* lineh, SVGRect* boundsOut, std::vector<SVGRect>* glyphPos)
{
    ...
    if(extraState().svgFont) {
        ...
        for(size_t ii = 0; ii < glyphs.size(); ++ii) {
            ...
            if(glyphPos)
                glyphPos->push_back(SVGRect::ltrb(pos.x, pos.y, pos.x+dx, pos.y));
        ...
        for(size_t ii = 0; *str; ++ii) {
            ...
            if(!textPath.empty()) {
                SVGPoint normal;
                SVGPoint pt = textPath.positionAlongPath(pos.x - textPathOffset, &normal);
                if(pt.isNaN())
...
SVGPoint SvgPainter::drawTextTspans(const SvgTspan* node, SVGPoint pos, real* lineh, SVGRect* boundsOut, std::vector<SVGRect>* glyphPos)
{
    ...
    if(node->tspans().empty())
        ...
        for(size_t ii = 0; ii < m_tspans.size(); ++ii) {
            ...
            if(m_tspans[ii]->isLineBreak()) {
                ...
                if(glyphPos)
                    glyphPos->push_back(SVGRect::centerwh(pos, 0, 0));
                pos.x = x0;
                ...
                *lineh = 0;
                ...
                if(!(states.textAnchor & Painter::AlignLeft)
                   ...
                   && textX.size() <= 1 && textY.size() <= 1) {
                    ...
                    SVGRect dummy;  // needed to suppress drawing
                    SVGPoint temppos = pos;
...
{
    Painter painter;
    std::vector<SVGRect> glyphpos = SvgPainter(&painter).glyphPositions(node);
...
void SvgPainter::elideText(SvgText* textnode, real maxWidth)
{
    ...
    textnode->addText("...");
    Painter painter;
    std::vector<SVGRect> glyphpos = SvgPainter(&painter).glyphPositions(textnode);
    textnode->clearText();
